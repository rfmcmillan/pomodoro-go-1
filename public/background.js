  ...
  init: async function () {
    try {
      if (!this.active) {
        ...
        storage.sync.clear();
        (0,_storage__WEBPACK_IMPORTED_MODULE_0__.setStoredIsRunning)(false); // setStoredTimer(null);
...
background.init(); //Timer in Background
...
chrome.alarms.onAlarm.addListener(alarm => {
  if (alarm.name === 'oneSecond') {
    chrome.storage.local.get(['isRunning', 'timer'], res => {
      ...
      if (time === 0) {
        (0,_storage__WEBPACK_IMPORTED_MODULE_0__.setStoredIsRunning)(false);
        return;
      }
